...
# 使用timm库创建预训练的EfficientNet B0模型
model = timm.create_model(model_name, pretrained=True, num_classes=2)
# 加载本地的预训练模型
# model.load_state_dict(torch.load('efficientnet_b2.pth'))
model = model.cuda()  # 将模型移到GPU上

epoch_num = 20  # 设置训练的总轮数
bs_value = 256  # 设置批次大小

# 定义损失函数和优化器
criterion = nn.CrossEntropyLoss().cuda()  # weight代表每个类别的权重
optimizer = torch.optim.Adagrad(model.parameters(), lr=0.001)
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epoch_num)  # 余弦退火学习率调度器  # 学习率调度器
...
for epoch in range(epoch_num):
    ...
    if val_acc.avg.item() > best_acc:
        ...
        # 保存新的最佳模型
        best_model_path = f'./model_{best_acc}_resnet18_with_color_jitter.pt'
